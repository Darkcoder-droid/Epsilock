...
## Real-time anomaly and enforcement tests

1. Message flood: send more than 8 messages in 10 seconds.
Expected: `MESSAGE_FLOOD` incident, socket disconnected, admin alert/beep.

2. WS event burst: spam 25+ WS events in 5 seconds.
Expected: `WS_EVENT_BURST` incident and socket disconnect.

3. Invalid room join: repeatedly join an unassigned room.
Expected: `ROOM_JOIN_DENIED`; repeated attempts trigger `INVALID_ROOM_JOIN` and block/reset enforcement.

4. Invalid room send: send packet with different `roomId` than joined room.
Expected: `INVALID_ROOM_SEND` incident and security action.

5. Oversized payload: send payload above policy threshold.
Expected: `OVERSIZED_PAYLOAD` incident and high-severity enforcement.

6. File abuse: send unsupported mime type or exceed file rate.
Expected: `FILE_ABUSE` incident.

7. Repeated failed login: fail login 5+ times.
Expected: `REPEATED_AUTH_FAILURES` incident and account block/reset requirement.

8. Abnormal disconnect: connect/disconnect repeatedly.
Expected: `ABNORMAL_DISCONNECT` incident.

9. Origin validation failure: connect from unapproved origin.
Expected: upgrade rejected and `ORIGIN_VALIDATION_FAILED` incident.

10. Token/session mismatch: reconnect with stale token after tokenVersion increment.
Expected: `TOKEN_SESSION_MISMATCH` incident and forced logout.

11. Admin disconnect: use “Disconnect User Now”.
Expected: all active sockets receive `FORCE_LOGOUT`.

12. Multi-device anomaly logout: same user logged in on node1 + node2, trigger high/critical anomaly.
Expected: both devices logout immediately.

13. Admin reset-password unlock:
Expected: user unblocked, `requirePasswordReset=false`, old sessions revoked, login only with new password.
