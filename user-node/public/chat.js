(() => {
  ...
  es.onmessage = (ev) => {
    ...
    if (msg.type === 'sendDenied') {
      ...
      if ((msg.payload.reason || '').toLowerCase().includes('frozen')) {
        state.roomJoined = false;
      }
      updateChatControls();
      return;
    ...
    if (msg.type === 'roomFrozen') {
      state.roomJoined = false;
      state.joiningRoom = false;
      pushLog(`Room frozen: ${msg.payload.reason}`);
      if (statusText) statusText.textContent = `Room frozen: ${msg.payload.reason}`;
      updateChatControls();
      return;
    }
